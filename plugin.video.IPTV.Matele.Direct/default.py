...
AddonID = 'video.IPTV.Matele.Direct'
...
mainURL = "https://raw.githubusercontent.com/Sphinxroot/plugin.video.IPTV.Matele.Direct/master/Menu"
...
def PlxCategory(url):
	...
	for channel in list[1:]:
		...
		if channel["type"] == 'playlist':
			AddDir("[COLOR gold][{0}][/COLOR]".format(name) ,channel["url"].encode("utf-8"), 1, iconimage, background=background.encode("utf-8"))
		else:
